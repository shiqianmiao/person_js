 ...
 */
var BMapLib = window.BMapLib = module.export = BMapLib || {};

//常量，infoBox可以出现的位置，此版本只可实现上下两个方向。
var INFOBOX_AT_TOP = BMapLib.INFOBOX_AT_TOP = 1,
    INFOBOX_AT_RIGHT = BMapLib.INFOBOX_AT_RIGHT= 2,
    INFOBOX_AT_BOTTOM = BMapLib.INFOBOX_AT_BOTTOM = 3,
    INFOBOX_AT_LEFT = BMapLib.INFOBOX_AT_LEFT = 4;
  ...
  /**
    ...
    baidu.object.extend(InfoBox.prototype,{
        ...
        _getCloseIcon: function(){
            if (this._opts.closeIconUrl == 'close.png') {
                var img = "<img src='"+ this._opts.closeIconUrl +"' align='right' style='display:none;position:absolute;right:0px;cursor:pointer;margin:"+ this._opts.closeIconMargin +"'/>";
            } else {
