...
Search.expand = function (config){
    ...
    $elem.click(function(e){
        $switchBtn = $(this);
        $(".js_expend").toggle($switchBtn.html()=="查看更多选项");
        if($switchBtn.html()=="查看更多选项"){
            $switchBtn.html("精简筛选条件");
            $(".js_expend").removeClass('hidden');
        } else{
            $switchBtn.html("查看更多选项");
            $(".js_expend").addClass('hidden');
        }
     });
};
...
function bindOptionClick() {
    ...
    $('.mileage  input[type="submit"]').on('click',function(e){
        //当前筛选条件名，如price
        var optionType = $('#option_type').val();
...
function bindBrandClick () {
    Common.anchor($('.area-list .letter'));
